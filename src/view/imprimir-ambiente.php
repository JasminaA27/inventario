...
curl_setopt_array($curl, array(
    CURLOPT_URL => BASE_URL_SERVER . "src/control/Ambiente.php?tipo=listarAmbientes&sesion=" . $_SESSION['sesion_id'] . "&token=" . $_SESSION['sesion_token'] ,
...
$respuestaAmbiente = $respuesta->ambientes;
...
$contador = 1;
foreach ($respuestaAmbiente as $ambiente) {
    $contenido_pdf .= "<tr>";
    $contenido_pdf .= "<td>{$contador}</td>";
    $contenido_pdf .= "<td>{$ambiente->institucionname}</td>";
...
$pdf->SetTitle('LISTA DE AMBIENTE');
...
ob_end_clean();
$pdf->Output('lista_ambientes.pdf', 'I');
?>
